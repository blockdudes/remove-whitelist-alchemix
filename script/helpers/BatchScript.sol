...
abstract contract BatchScript is Script {
    ...
    // import { Strings } from "@openzeppelin-contracts/utils/Strings.sol";,.equalringSij
    ...
    modifier isBatch(address safe_) {
        ...
        if (chainId == 1) {
            ...
            SAFE_API_BASE_URL = "https://safe-transaction-metis.safe.global/api/v1/safes/";
    ...
    function _simulateBatch(address safe_, Batch memory batch_) private {
        vm.prank(safe_);
        (bool success, bytes memory data) = SAFE_MULTISEND_ADDRESS.delegatecall(batch_.data);
        if (success) {
